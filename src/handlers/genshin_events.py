        ...
        async with aiohttp.ClientSession() as httpsession:
            for news_channel_id in conf.NEWS_CHANNEL_IDS:
                ...
                async for message in channel.history(limit=100):
                    if source and message.created_at <= source.read_until.replace(tzinfo=pytz.UTC):
                        break
                    if not latest:
                        latest = message.created_at
                    for embed in message.embeds:
                        for url in re.findall(r"https://[A-z0-9./?#]+", embed.description):
                            async with httpsession.get(url) as response:
                                if (response.url.host == "webstatic-sea.mihoyo.com"
                                        and response.url.path.startswith("/ys/event/")):
                                    events.append(genshin_events.GenshinEvent(
                ...
                for event in events:
                    ...
                    if not existing:
                        session.add(event)
                        for event_channel in session.execute(select(GuildSettings).where(GuildSettings.key == GuildSettingKey.EVENT_CHANNEL)).scalars():
                            embed = discord.Embed(description=event.description)
                            channel = await self.bot.fetch_channel(event_channel.value)
                            event_role = session.get(GuildSettings, (event_channel.guild_id, GuildSettingKey.EVENT_ROLE))
                            await channel.send(content=f"\n<@&{event_role.value}>" if event_role else None, embed=embed)
                        session.commit()

                source = genshin_events.EventSource(channel_id=news_channel_id, read_until=latest)
