...
class EmoteHandler(commands.Cog):
    ...
    async def emotes(
        ...
        message_id: Option(
            str, "The ID of the message that contains the emotes or stickers"
        ),
    ):
        ...
        for emote_name, emote_id in emotes:
            lines += [
                f"Emote [{emote_name}](https://cdn.discordapp.com/emojis/{emote_id}.png?size=1024)"
            ]
        for emote_name, emote_id in animated_emotes:
            lines += [
                f"Emote [{emote_name}](https://cdn.discordapp.com/emojis/{emote_id}.gif?size=1024&quality=lossless)"
            ]
        for sticker in message.stickers:
            lines += [
                f"Sticker [{sticker.name}](https://media.discordapp.net/stickers/{sticker.id}.png?size=1024)"
            ]
